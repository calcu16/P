...
#define OTHER(n) (1 - (n))
#define out (ret)
#define CHECK_ARGS(t) (assert(TRUE))
struct node
{
    void* value_;
    node *children_[2];
                        ...
                        node * const this, void * const item, node *out)
{
    ...
    {
        // zig-zag
        rotate(splay, out->children_[child], &out->children_[child]);
        rotate(child, out, &out);
    }
    return NO_ROTATE;
}
                        ...
                        node * const root, void * const item, node *out)
{
    int search;
    CHECK_ARGS(node*);
    search = search_node(compare, insert, root, item, &out);
    if(search == NOT_FOUND)
        return 0;
    if(search == NO_ROTATE)
...
int main(void)
{
    ...
    print_tree(start);
    insert(start, numbers + 1, &start);
    print_tree(start);
    insert(start, numbers + 3, &start);
    print_tree(start);
    insert(start, numbers + 5, &start);
    print_tree(start);
    insert(start, numbers + 7, &start);
    print_tree(start);
    insert(start, numbers + 0, &start);
    print_tree(start);
    insert(start, numbers + 4, &start);
    print_tree(start);
    find(start, numbers + 7, &start);
    print_tree(start);
    find(start, numbers + 2, &start);
    print_tree(start);
    delete(start, numbers + 6, &start);
    print_tree(start);
    delete(start, numbers + 0, &start);
    print_tree(start);
    return 0;
}
